...
model User {
  ...
  country       String?  // ISO 3166-1 alpha-2 code (e.g., "US", "GB", "IN") - for leaderboards
  isPublic      Boolean  @default(true) @map("is_public")
  profileReadme String?  @db.Text @map("profile_readme")
  createdAt     DateTime @default(now()) @map("created_at")
  updatedAt     DateTime @updatedAt @map("updated_at")

  // Core music relations
...
model Powerup {
  ...
  icon            String
  effectType      String   @map("effect_type") // freeze_time, add_time, multiplier, block_penalty, remove_options, skip_question
...
model PowerupPurchase {
  ...
  discount    Float    @default(0) // 0.10 = 10% off
...
model LeaderboardHistory {
  ...
  topEntries  Json     @map("top_entries") // Top 10 archived
  createdAt   DateTime @default(now()) @map("created_at")
...
model Badge {
  id          String   @id @default(cuid())
  userId      String   @map("user_id")
  badgeType   String   @map("badge_type") // first_quiz, speed_demon, perfectionist, streak_5, etc.
  artistId    String?  @map("artist_id") // For artist-specific badges
...
model Concert {
  ...
  eventType         String   @map("event_type") // concert, festival, tour, etc.
  venueId           String   @map("venue_id")
  date              DateTime
  doorsOpen         String?  @map("doors_open")
  startTime         String?  @map("start_time")
  timezone          String
  ticketStatus      String   @map("ticket_status") // on_sale, presale, sold_out, etc.
  ...
  sourceName        String   @map("source_name") // bandsintown, songkick, etc.
...
model Venue {
  ...
  longitude    Float
  capacity     Int?
  venueType    String?  @map("venue_type") // stadium, arena, theater, club, outdoor
  imageUrl     String?  @map("image_url")
  website      String?
  createdAt    DateTime @default(now()) @map("created_at")
...
model TicketSource {
  ...
  name         String  // Ticketmaster, StubHub, AXS, etc.
  url          String
  minPrice     Float?  @map("min_price")
  maxPrice     Float?  @map("max_price")
...
model UserLocation {
  ...
  latitude     Float?
  longitude    Float?
  searchRadius Int      @default(100) @map("search_radius") // km
...
model ConcertReminder {
  ...
  userId       String   @map("user_id")
  concertId    String   @map("concert_id")
  reminderType String   @map("reminder_type") // day_before, week_before, on_sale
...
model ScrapingJob {
  id             String    @id @default(cuid())
  type           String    // artist_concerts, location_concerts, full_refresh
